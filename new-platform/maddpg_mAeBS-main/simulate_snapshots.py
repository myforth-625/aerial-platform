...
def save_snapshot(step, nodes, terminals, ref_lon, ref_lat, max_bound_lon, max_bound_lat):
    output_json = f"过程快照/-1/snapshot_step_{step}.json"
    output_png = f"过程快照/-1/snapshot_step_{step}.png"
    ...
    ax.scatter(lons_t, lats_t, [0]*len(terminals), c='blue', s=10, alpha=0.3, label='Terminals')
    ...
    ax.set_ylabel('Lat')
    ax.set_zlabel('Alt')
    ax.set_title(f"UAV Deployment Snapshot - Step {step}")
    plt.legend()
    plt.savefig(output_png)
    plt.close()
    print(f"Saved snapshot to {output_json} and {output_png}")

def simulate():
    input_nodes_path = "过程快照/-1/aerialNode.json"
    input_terminals_path = "过程快照/-1/terminalSnapshot_new.json"
    model_dir = "models"
    snapshot_steps = [30, 50, 80, 100]
    ...
    print(f"Starting simulation for snapshots...")
    ...
    for step in range(1, max_steps + 1):
        ...
        if step in snapshot_steps:
            save_snapshot(step, current_nodes, terminals, ref_lon, ref_lat, max_bound_lon, max_bound_lat)
