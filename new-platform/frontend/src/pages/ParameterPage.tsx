...
function normalizePnType(raw: unknown): NodeType | null {
  ...
  if (key === 'GROUND_STATION' || key === 'GS' || key === 'GROUNDSTATION') return 'GROUND_STATION';
...
function parseImportedJson(raw: string): { ok: true; nodes: NodeData[] } | { ok: false; error: string } {
  let parsed: any;
  try {
  ...
  for (let i = 0; i < (list as any[]).length; i++) {
    ...
    const type = normalizePnType(item.pnType ?? item.type);
    if (!type) continue;
    const lng = parseNumericField(item.longitude ?? item.lng);
    const lat = parseNumericField(item.latitude ?? item.lat);
    const height = parseNumericField(item.height);
    if (lng === null || lat === null) continue;
    const id = String(item.pnId ?? item.id ?? `${type}-${i}`);
    out.push({
      id,
      type,
      lng,
  ...
  if (out.length === 0) {
    return { ok: false, error: 'No valid nodes recognized' };
  }
  ...
  const uploadProps: UploadProps = {
    ...
    beforeUpload: file => {
      ...
      reader.onload = e => {
        ...
        message.success(`Imported ${result.nodes.length} nodes`);
      };
